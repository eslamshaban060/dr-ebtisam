...
import { enArticles } from "../../../../components/ar-components/dashboard/components/data";
const newEl = {
  id: 11,
  image: Avatar2,
  title: "Lorem ipsum dolor sit amet consectetur",
  description:
    "        Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae atque fuga vitae itaque excepturi repellendus sunt odio nemo esse accusantium. Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae atque fuga vitae itaque excepturi repellendus sunt odio nemo esse accusantium.",
  date: " November 21, 2023",
};

const ArticlePage = () => {
  const [myArticles, setMyArticles] = useState([...enArticles]);
  const latestArticle = myArticles.slice(-4);
  const add = () => {
    setMyArticles([...enArticles, newEl]);
        ...
        >
          <div className="box flex bg-[var(--wh)] lg:w-[clamp(300px,40dvw,100%)] w-[clamp(300px,80dvw,100%)] rounded-3xl h-80 p-10 gap-10 items-center justify-evenly flex-col ">
            <p className=" w-full lg:w-[100%] text-shadow-lg text-center text-[var(--nv)] font-medium lg:text-3xl text-2xl md:leading-12">
              Click the button to go to Blogger and write a new article.{" "}
            </p>
      ...
      <section className="h-[75dvh] overflow-y-auto  bg-[var(--lb)] p-2">
        ...
        <div className=" article-message border w-full  min-h-[256px] md:py-0 py-4 rounded-2xl ">
          <div className="w-full h-full flex justify-center gap-5 items-center px-7   md:flex-row flex-col ">
            <div className="image relative flex justify-center items-center z-10 md:order-2 p-4 flex-1">
              <img
                src={Avatar2.src}
                alt="..."
            ...
            <div className="text sm:text-2xl text-lg text-[var(--wh)] font-semibold text-center flex-2 ">
              Welcome back, <span className="text-[var(--yl)]">Doctor.</span>{" "}
              Your dashboard is designed to help you manage everything easily
              and efficiently!
            </div>
          </div>
        </div>
        <div className="actions w-full mt-16 flex flex-col gap-10 justify-center items-center ">
          ...
          <div className="buttons w-full p-5 grid gap-16 sm:grid-cols-[repeat(auto-fit,minmax(250px,1fr))] grid-cols-[repeat(auto-fit,minmax(200px,1fr))]">
            <button
              onClick={() => alert("sdjfi")}
              className="order-2 bg-img-1 sm:p-10 p-1 h-48 flex  active:scale-[0.9] transition rounded-2xl items-center justify-center font-semibold md:text-2xl text-lg text-shadow-lg text-[var(--wh)] "
            >
              Discover all your medical articles here
            </button>
            <button
            ...
            {latestArticle.map((article) => (
              ...
              >
                ...
                <div className="text md:px-10 px-2 py-5 flex  justify-between flex-col flex-2">
                  ...
                  <div className="date text-right md:text-sm text-[12px] ">
                    {article.date}
                  </div>
                </div>
              </div>
